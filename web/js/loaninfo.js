$('#myModal').on('show.bs.modal', function (event) {
    ...
    $.get('index.php?r=site/loaninfo', {loanid: loanid}, function (data) {
        ...
        for (var i=0; i < len; i++) {
            ...
            $("#pay-list").append("<tr bgcolor='" + color +"'><td>"+ loaninfo[4][i].paydate +"</td><td>"+ loaninfo[4][i].payamount +"</td><td>"+ (loaninfo[4][i].delamt).toFixed(2) +"</td><td>"+ (loaninfo[4][i].penalty).toFixed(2) +"</td><td>"+ 0 +"</td><td>"+ (loaninfo[4][i].balance).toFixed(2) +"</td></tr>");
